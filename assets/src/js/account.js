...
(function () {
  ...
  function initPreviewToggle() {
    var toggle = document.querySelector("[data-pks-oi-preview-toggle]");
    var device = document.querySelector("[data-pks-oi-preview-device]");
    if (!toggle || !device) {
      return;
    }
    toggle.addEventListener("click", function () {
      device.classList.toggle("is-full-width");
    });
  }
  ...
  function initDesignEditor() {
    ...
    function setStatus(message, isError) {
      ...
      statusEl.setAttribute(
        "aria-busy",
        message && !isError && message.indexOf("…") > -1 ? "true" : "false"
      );
    }
